...
class TISFM(TemplateModel):
  def __init__(self, num_of_response, motif_path, window_size, 
    ...
    self.and_layer = Linear(in_features=self.out_channels//2, out_features=self.out_channels//2, bias=True)
    self.or_layer = Linear(in_features=self.out_channels//2, out_features=self.out_channels//2, bias=True)

    self.linreg = Linear(in_features=self.linreg.in_features * 2, out_features=self.linreg.out_features)
  ...
  def forward(self, x):
    ...
    andgate = self.sigmoid(self.and_layer(x))
    orgate = self.sigmoid(self.or_layer(x))
    x = torch.cat([x * andgate, x + orgate], dim=1)
