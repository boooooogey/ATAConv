...
class TISFM(TemplateModel):
    ...
    def __init__(self, num_of_response,
        ...
        super(TISFM, self).__init__(num_of_response, motif_path, window_size, stride, pad_motif,
                                    both_direction=False, initiate_conv_w_cdf=True)

        self.motif_activation = torch.nn.Sigmoid()
        self.activation = torch.nn.LeakyReLU()#torch.nn.GELU()
        self.activation_out = torch.nn.LeakyReLU()
        self.motif_scaler = torch.nn.Parameter(torch.empty(self.out_channels, requires_grad = True))
        with torch.no_grad():
            torch.nn.init.ones_(self.motif_scaler)
    ...
    def forward(self, x):
        ...
        x = self.motif_activation(x)

        x = einops.einsum(x, self.motif_scaler, "b c l, c -> b c l")
