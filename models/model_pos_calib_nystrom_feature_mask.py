...
class TISFM(TemplateModel):
    ...
    def __init__(self, num_of_response,
        ...
        self.mask_weights = torch.nn.Parameter(torch.empty(self.out_channels // 2, 32))
        torch.nn.init.xavier_uniform_(self.mask_weights)
    ...
    def forward(self, x):
        ...
        mask, _ = self.nystrom_layer(x)

        mask = self.sigmoid(einsum(mask, self.mask_weights, "b m f, m f -> b m"))
